...
Ext.define( 'EnvironmentManager', {
  ...
  /**
  get_path: function( row,arry ){
    if(row.parentNode != undefined && row.parentNode != null){
      this.get_path( row.parentNode,arry );
    }
    arry.push(row.get( 'name' ));
    return arry;
  },
  */
  ...
  constructor: function( config ){
    ...
    var store = Ext.create('Ext.data.TreeStore', {
      ...
      proxy: {
        type: 'ajax',
        url: "/data_elements/tree.json?service_id="+ this.service_id
      },
      folderSort: true
    });
